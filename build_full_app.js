...
// Build the complete clean index.html
...
// DATA & CONSTANTS
// ============================================================
const COURSE_DATA = ${fs.readFileSync('scratch_course_data.json', 'utf8')};
...
// ============================================================
let students = [
  {nama:"Aysha",progress:"Aysha learned about creating AR games on Delightex with animal themes. Also studied drone logic on Tynker.\n\nNote: Aysha completed all tasks very well and is starting to understand loop concepts."},
  {nama:"Puan",progress:"Absent (Excused)"}
];
...
function buildAutoWAMessage(){
  const tgl = document.getElementById('auto-tanggal')?.value;
  const tglIndo = formatDateLong(tgl);
  const studentListText = autoStudents
    .filter(s => s.nama || s.progress)
    .map(s => `*${s.nama || '—'}*\n${s.progress || '—'}`)
    .join('\n\n');
...
console.log('Successfully wrote clean index.html! File size:', fullHtml.length);
